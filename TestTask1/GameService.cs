...
namespace GameServer
{
    public class GameService : GameServiceProto.GameServiceProtoBase
    {
        ...
        public GameService(GameServiceDbContext dbContext, IUserRepository repositoryUser)
        {
        ...
        public override async Task<RpcStatus> CreateUser(UserRequest request, ServerCallContext context)
        {
            RpcStatus rpcStatus = new RpcStatus();
            ...
            var user = await _repositoryUser.Create(userRequest);

            if (user == null)
            {
                rpcStatus.Message = "Пользователь не создан";
                return rpcStatus;
            }

            rpcStatus.Message = $"Пользователь создан ID - {user}";
            return rpcStatus;
        }
        ...
        public override async Task<BalanceResponse> GetBalance(PlayerId playerId, ServerCallContext context)
        {
            var user = await _dbContext.User
            .SingleOrDefaultAsync(m => m.UserId == playerId.Id);
